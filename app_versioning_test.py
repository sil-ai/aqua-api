...
def get_latest_routers():
    ...
    base = list(
        filter(
            lambda row: re.search(r"^(?!.*prefix.*).*include_router\((.*)\)$", row),
            app_code,
        )
    )
    latest_routers = []
    for base_row, latest_row in zip(base, latest):
        ...
        latest_router_name = re.search(
            r"include_router\((.*), prefix", latest_row
        ).groups()[0]
...
def get_first_routers():
    ...
    return [
        re.search(r"include_router\((.*), prefix", item).groups()[0]
        for item in first_routers
    ]
...
def get_non_latest():
    ...
    latest_versions = [
        tuple(["_".join(item.split("_")[:2])] + item.split("_")[-1:])
        for item in latest_routers
    ]
    app_code = get_app_code()
    all_versioned_routers = list(
        filter(lambda item: re.search(r'prefix="/v', item), app_code)
    )
    non_latest = []
    for router, version in latest_versions:
        ...
        non_latest.append(
            list(
                filter(lambda row: re.search(filter_regex, row), all_versioned_routers)
            )[0]
        )
    print(non_latest)
    return [
...
def test_latest_routers(imports, base_router, latest_router):
    ...
    latest_import_version = max(
        [int(re.search(r"\.v(.*?)\.", item).groups()[0]) for item in this_router]
    )
    # should be the v2 that we injected earlier
    assert latest_import_version == int(version)


@pytest.mark.parametrize("first_router", get_first_routers(), ids=get_first_routers())
...
@pytest.mark.parametrize("non_latest", get_non_latest(), ids=get_non_latest())
