...
def test_token_generation(test_db_session):
    response = client.post(f"{prefix}/token", data={"username": "testuser", "password": "password"})
    assert response.status_code == 200
    assert "access_token" in response.json()

def test_unauthorized_token_generation(test_db_session):
    response = client.post(f"{prefix}/token", data={"username": "testuser", "password": "wrongpassword"})
