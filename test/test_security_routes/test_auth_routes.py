...
def test_token_generation(test_db_session):
    response = client.post(f"{prefix}/token", data={"username": "testuser1", "password": "password1"})
    assert response.status_code == 200
    assert "access_token" in response.json()

def test_unauthorized_token_generation(test_db_session):
    response = client.post(f"{prefix}/token", data={"username": "testuser1", "password": "wrongpassword"})
...
def test_read_current_user(test_db_session):
    ...
    response = client.get(f"{prefix}/users/me", headers={"Authorization": f"Bearer {test_token}"})
    assert response.status_code == 200
