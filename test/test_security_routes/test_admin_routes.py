...
def group_exists(db_session, groupname):
    return (
        db_session.query(GroupDB)
        .filter(GroupDB.name == groupname)
        .count()
        > 0
    )
...
def test_admin_flow(client, regular_token1, admin_token, test_db_session):
    ...
    assert response.json()["message"] == f"User {link_data['username']} successfully linked to group {link_data['groupname']}"
    ...
    assert not link_exists(test_db_session, link_data["username"], link_data["groupname"])    
