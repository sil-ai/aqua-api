...
def run_total_scores(
    ...
    all_results.loc[:, 'match_score'] = all_results.merge(match_scores_df, how='left', on=['source', 'target'])
    # all_results.loc[:, "match_score"] = faster_df_apply(all_results, lambda x: get_scores_from_match_dict(match_scores_df, x["source"], x["target"], normalized=False)[0]).astype('float16')
