...
def get_correlations_between_sets(
    indexes_set_1: set, indexes_set_2: set
) -> Tuple[float, int]:
    """
...
def update_matches_for_lists(
    ...
    for source_item in source_list:
        if source_item.normalized in matches:
            continue
...
def run_match_scores(
    ...
    condensed_df.loc[:, "src_list"] = condensed_df["src"].apply(
        lambda x: str(x).split()
    )
    condensed_df.loc[:, "trg_list"] = condensed_df["trg"].apply(
        lambda x: str(x).split()
    )
    condensed_df.loc[:, "normalized_src_words"] = condensed_df["src"].apply(
        lambda x: prepare_data.normalize_word(x).split()
    )
    condensed_df.loc[:, "normalized_trg_words"] = condensed_df["trg"].apply(
        lambda x: prepare_data.normalize_word(x).split()
    )

    condensed_df_indexes = list(condensed_df.index)
    for word_object in word_dict_src.values():
        word_object.reduced_index_list = list(
            set(word_object.index_list).intersection(set(condensed_df_indexes))
        )
    ...
    for _, row in condensed_df.iterrows():
        paired_source_objects: List[prepare_data.Word] = list(
            set([word_dict_src[x] for x in row["src_list"]])
        )
        paired_target_objects: List[prepare_data.Word] = list(
            set([word_dict_trg[x] for x in row["trg_list"]])
        )
        matches = update_matches_for_lists(
    ...
    flat_list = [{"source": k, **x} for k, v in matches.items() for x in v]
    if len(flat_list) == 0:
