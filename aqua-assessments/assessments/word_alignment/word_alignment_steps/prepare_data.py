...
def normalize_word(word: str) -> str:
    ...
    """
    word_norm = (
        re.sub(r"[^\w\s]", "", str(word).lower()) if word else ""
    )  #  Gives 18,159 unique Hebrew words in the OT, rather than 87,000
    if len(word_norm) == 0:
        return word
    return word_norm
...
def create_tokens(src_data: List[str], vref_filepath: Path):
    ...
    tokenized_list = list(
        tokenized_corpus.align_rows(tokenized_corpus).lowercase().to_pandas()["source"]
    )
    condensed_df.loc[:, "src_tokenized"] = tokenized_list.copy()
    ...
    for index, row in df[:1:-1].iterrows():
        if row["range_next"]:
            ...
            if len(df.loc[index + 1, "src"].replace("< range >", "")) > 0:
                df.loc[index, "src"] += " " + df.loc[index + 1, "src"].replace(
                    "< range >", ""
                )
            if len(df.loc[index + 1, "trg"].replace("< range >", "")) > 0:
                df.loc[index, "trg"] += " " + df.loc[index + 1, "trg"].replace(
                    "< range >", ""
                )
