...
def run_total_scores(
    ...
    top_source_scores_df    A dataframe with the top score for each vref-source
    ...
    condensed_df = condensed_df.drop(["src", "trg"], axis=1)
    condensed_df = condensed_df.explode("source").explode("target")
    alignment_scores_df["vref"] = alignment_scores_df["vref"].astype(
        "object"
    )  # Necessary for non-Bible, where vrefs are ints.

    alignment_scores_df = alignment_scores_df.merge(
        avg_alignment_scores_df, how="left", on=["source", "target"]
    ).fillna(0)
    print("Merging alignment scores")
    all_results = condensed_df.merge(
        alignment_scores_df, how="left", on=["vref", "source", "target"]
    ).fillna(0)

    # Remove duplicates
    all_results = all_results.groupby(
        ["vref", "source", "target"], sort=False, as_index=False
    ).mean()

    print("Merging translation scores")
    all_results = all_results.merge(
        translation_scores_df, how="left", on=["source", "target"]
    )

    all_results.loc[:, "avg_aligned"] = all_results.apply(
        lambda row: row["alignment_count"] / row["co-occurrence_count"], axis=1
    ).astype("float16")
    all_results.loc[:, "translation_score"] = (
        all_results.loc[:, "translation_score"]
        .apply(lambda x: 0 if x < 0.00001 else x)
        .astype("float16")
    )
    ...
    match_scores_df = match_scores_df.rename(
        columns={"source": "source_norm", "target": "target_norm"}
    )
    print("Merging match scores")
    all_results = all_results.merge(
        match_scores_df, how="left", on=["source_norm", "target_norm"]
    )
    # Drop the normalized columns
    all_results = all_results.drop(["source_norm", "target_norm"], axis=1)
    print("Merging embedding scores")
    all_results = all_results.merge(
        embedding_scores_df, how="left", on=["source", "target"]
    )

    print("Calculating total scores")
    all_results.loc[:, "total_score"] = all_results[
        [
            "avg_aligned",
            "translation_score",
            "alignment_score",
    ...
    top_source_scores_df = total_scores_df.loc[
        total_scores_df.groupby(["vref", "source"], sort=False)["total_score"].idxmax(),
        :,
    ].reset_index(drop=True)
    top_target_scores_df = total_scores_df.loc[
        total_scores_df.groupby(["vref", "target"], sort=False)["total_score"].idxmax(),
        :,
    ].reset_index(drop=True)
    print("Calculating verse scores")
    verse_scores_df = top_source_scores_df.groupby(
        "vref", as_index=False, sort=False
    ).mean()
    verse_scores_df = verse_scores_df.fillna(0)

    all_results = (
        all_results if return_all_results else pd.DataFrame()
    )  # It's a big dataframe, so only return it if requested.
