...
def run_embeddings(
    condensed_df: pd.DataFrame, source_index_cache: dict, target_index_cache: dict
) -> pd.DataFrame:
    """
    run_embeddings
    ...
    if df.shape[0] > 0:
        df.loc[:, "embedding_dist"] = df.apply(
            lambda row: word_dict_src[row["src_list"]].get_norm_distance(
                word_dict_trg[row["trg_list"]]
            ),
            axis=1,
        ).astype("float16")
        df.loc[:, "embedding_score"] = (
            df["embedding_dist"]
            .apply(lambda x: math.log1p(max(1 - x, -0.99)))
            .astype("float16")
        )
    df = df[["src_list", "trg_list", "embedding_score"]].rename(
        columns={"src_list": "source", "trg_list": "target"}
    )
