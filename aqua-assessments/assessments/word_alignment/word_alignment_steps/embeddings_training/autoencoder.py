...
def X_gen(
    word_dict: Dict[str, dict], languages: List[str], batch_size: int = 32
        ...
        for i in range(len(word_dict[language]) // batch_size):
            yield torch.tensor(
                np.array(
                    [
                        word.index_ohe
                        for word in words[i * batch_size : (i + 1) * batch_size]
                    ]
                )
            ).float().to(dev)
...
def run_training(
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Epoch:{epoch+1}, Train loss:{epoch_train_loss.mean():.6f}, Validation loss:{epoch_val_loss.mean():.6f}"
        )
        clearml.Logger.current_logger().report_scalar(
            "training/validation loss",
            "training loss",
            iteration=epoch + 1,
            value=epoch_train_loss.mean(),
        )
        clearml.Logger.current_logger().report_scalar(
            "training/validation loss",
            "validation loss",
            iteration=epoch + 1,
            value=epoch_val_loss.mean(),
        )
...
def train_model(
    ...
    clearml.Task.init(
        ...
        task_name="autoencoder-train-"
        + str(int(time.time())),  # task name of at least 3 characters
        task_type="training",
        tags=None,
...
async def get_index_cache(source: Path, cache_dir: Path, refresh: bool = False):
    """
...
async def main(args):
    ...
    for lang, lang_path in {**training_language_paths, **val_language_paths}.items():
        index_cache = await get_index_cache(
            lang_path, args.cache_dir, refresh=args.refresh_cache
        )
    ...
    torch.save(
        model.state_dict(), outpath / f"model_{len(training_language_paths)}_{timestr}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=Path,
        help="A list of texts to train the model on.",
    )
    parser.add_argument(
        "--val-langs",
        nargs="+",
        type=Path,
        help="A list of texts to use for validation scores.",
    )
    ...
    parser.add_argument(
        "--batch-size", type=int, help="Batch size for training", default=128
    )
    ...
    parser.add_argument(
        "--hidden-sizes", nargs="+", type=int, help="Hidden sizes", default=None
    )
    parser.add_argument(
        "--lr", type=float, help="Learning rate for training", default=0.001
    )
    parser.add_argument(
        "--weight-decay", type=float, help="Weight decay for training", default=1e-6
    )
    parser.add_argument("--outpath", type=Path, help="where to store results")
    parser.add_argument(
        "--refresh-cache", action="store_true", help="Refresh the cache of match scores"
    )
