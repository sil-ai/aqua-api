...
def merge_verse_ranges(
    ...
    When a verse has "<range>" in any of the specified text fields, it indicates that
    ...
    text_fields : List[str], optional
        List of field names that should be checked for <range> marker and concatenated
        when merging. If None, will automatically detect fields containing "<range>".
    ...
    combine_fields : callable, optional
    ...
    List[Dict[str, Any]]
        ...
        - Combined values for each text field (using combine_fields function)
        - Only vrefs field and text fields specified in text_fields parameter
    ...
    ... ]
    >>> result = merge_verse_ranges(verses, text_fields=["score"], is_range_marker=lambda x: x == 0)
    >>> result[0]["vrefs"]
    ['GAL 1:1', 'GAL 1:2']
    ...
    if combine_fields is None:
        ...
        combine_fields = default_combine
    ...
    if text_fields is None:
        ...
        if verses:
            ...
            text_fields = [k for k in verses[0].keys() if k != verse_ref_field]
    ...
    for verse in verses:
        # Check if any text field has range marker
        has_range = any(is_range_marker(verse.get(field)) for field in text_fields)
    ...
    for group in verse_groups:
        if len(group) == 1:
            ...
            merged = _merge_group(
                group, verse_ref_field, text_fields, is_range_marker, combine_fields
            )
...
def _merge_group(
    ...
    for field in text_fields:
        ...
        if values:
            merged[field] = combine_fields(field, values)
        else:
            merged[field] = ""
