...
stub = modal.Stub(name="runner" + suffix, image=modal.Image.debian_slim().pip_install("boto3==1.26.56"))
...
@stub.function(timeout=7200, secrets=[modal.Secret.from_name("aqua-db"),modal.Secret.from_name("my-aws-secret")], dependencies=[Depends(api_key_auth)], mounts=modal.create_package_mounts(['key_fetch']))
...
@stub.webhook(method="POST", secrets=[modal.Secret.from_name("aqua-db"),modal.Secret.from_name("my-aws-secret")], dependencies=[Depends(api_key_auth)], mounts=modal.create_package_mounts(['key_fetch']))
